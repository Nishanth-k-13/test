...
def test_seo_data_extraction(page: Page, url: str):
    global worker_url_count
    worker_url_count += 1
    
    if worker_url_count > 1 and worker_url_count % URLS_BEFORE_SLEEP == 0:
        print(f"Worker processed {worker_url_count-1} URLs. Sleeping for {SLEEP_INTERVAL} seconds...")
    ...
    try:
        ...
        with open(OUTPUT_CSV, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                result["URL"], result["Final URL"], result["Redirected"], result["Status Code"],
