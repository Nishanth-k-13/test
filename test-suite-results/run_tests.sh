...
VENV_DIR="../test-suite-balance-sheet/.venv"
if [[ ! -d "${VENV_DIR}" ]]; then
  echo "Virtual environment not found in ../test-suite-balance-sheet/.venv"
  exit 1
fi
...
read -r WORKERS SLEEP_INTERVAL URLS_BEFORE_SLEEP RERUN_FAILURES FAILED_URLS_LOG <<< "$(
  python -c "import test_results_pages as m; print(m.WORKERS, m.SLEEP_INTERVAL, m.URLS_BEFORE_SLEEP, m.RERUN_FAILURES, m.FAILED_URLS_LOG)"
)"
...
echo ""
echo "Running Financial Results page test suite..."
