...
def test_seo_data_extraction(page: Page, url: str):
    ...
    try:
        ...
        with allure.step("Extract SEO Data"):
            ...
            result["OG"] = og_locator.first.get_attribute("content") if og_locator.count() > 0 else "N/A"
            
            tw_locator = page.locator("meta[name='twitter:title']")
            result["TW"] = tw_locator.first.get_attribute("content") if tw_locator.count() > 0 else "N/A"
